...
const _primary = Color(0xFFFBAC1A);
const _secondary = Color(0xFFFFFFFF);
...
final _darkPrimaryContainer = _primary.withOpacity(0.2);
final _darkSecondaryContainer = _secondary.withOpacity(0.2);
...
ThemeData _getTheme({required Brightness brightness}) {
  ...
  final buttonTextStyle = textTheme.bodyLarge;
...
TextTheme _getTextTheme(ColorScheme colorScheme) {
  ...
  final displayColor = isDark ? colorScheme.onBackground : _darker;
  const displayWeight = FontWeight.w500;
  const displayHeight = 1.2;
  const displayLetterSpacing = 0.0;



  final headlineColor = isDark ? colorScheme.onBackground : _darker;
